...
async function generateTotalsBoard() {
	...
	for (const pointType of pointTypes) {
		const thisTypePoints = points.filter((point) => point.type === pointType.id);

		const typeTotalPoints = thisTypePoints.reduce((acc, point) => acc + point.value, 0);
		const miles = typeTotalPoints / pointType.pointsPerMile;
		const kilometers = miToKm(miles);

		typeStrings.push(
			`## ${pointType.name}
			- ${typeTotalPoints} points (${Math.round(100 * typeTotalPoints/totalPoints)}% of total)
